...
- Frontend: `npx tsc --noEmit` shows no new errors (3 pre-existing, unrelated to files touched); `npx vitest run` — all touched-page test files pass (`DashboardPage`, `ComparePage`, `RunPanel`, `CyclesPage`: 10/10). Manually verified in-browser against the live `sample-all-frameworks-stochastic` run history: Dashboard shows a health bar per recent run; Compare's Outcome Mix renders a green baseline bar next to a green/red/yellow current bar for a real regression.
- Backend: `pytest tests/unit/testo_core -k "config or loader or pyramid or stage_tier or report"` — 116 passed, 1 skipped. `testo report pyramid d513f43b-8c56-4820-9f22-ad8983b9f752` against the live DB printed a real ASCII pyramid (`unit=130 integration=34 e2e=34 shape=irregular`) for `sample-all-frameworks-stochastic`.
