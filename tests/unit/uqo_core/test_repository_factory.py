...
from uqo_core.repository.factory import (
...
def test_postgresql_adapter_requires_driver(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("uqo_core.repository.factory.find_spec", lambda name: None if name == "psycopg" else object())
...
def test_mysql_adapter_requires_driver(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("uqo_core.repository.factory.find_spec", lambda _name: None)
